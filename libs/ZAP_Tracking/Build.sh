...
$KMS_VERSION $VERSION_H CreatePackages.sh Export.sh Scripts/Enable.sh
if [ 0 != $? ] ; then
    echo ERROR  $KMS_VERSION $VERSION_H CreatePackages.sh Export.sh Scripts/Enable.sh  failed
    exit 30
fi
